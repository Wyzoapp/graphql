...
namespace Wyzo\GraphQLAPI\Mutations\Admin\Catalog\Attributes;

use App\Http\Controllers\Controller;
use Illuminate\Support\Facades\Event;
use Nuwave\Lighthouse\Support\Contracts\GraphQLContext;
use Wyzo\Attribute\Repositories\AttributeOptionRepository;
use Wyzo\Attribute\Repositories\AttributeRepository;
use Wyzo\Core\Rules\Code;
...
class AttributeMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            if (
                ...
                $localeFields = wyzo_graphql()->manageLocaleFields($args['translations'], $this->localeFields);
            ...
            if (
                ...
                foreach ($attribute->options as $option) {
                    if (
                        ...
                        wyzo_graphql()->uploadImage($option, $swatch_value[$option->admin_name], 'attribute_option/', 'swatch_value');
                    }
                }
            }
            ...
            return [
                'success'   => true,
                'message'   => trans('wyzo_graphql::app.admin.catalog.attributes.create-success'),
                'attribute' => $attribute,
            ];
        } catch (\Exception $e) {
    ...
    public function update(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $attribute) {
            throw new CustomException(trans('wyzo_graphql::app.admin.catalog.attributes.not-found'));
        }
        ...
        try {
            ...
            return [
                'success'   => true,
                'message'   => trans('wyzo_graphql::app.admin.catalog.attributes.update-success'),
                'attribute' => $attribute,
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $attribute->is_user_defined) {
            throw new CustomException(trans('wyzo_graphql::app.admin.catalog.attributes.user-define-error'));
        }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.catalog.attributes.delete-success'),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
        }
    ...
    public function manageAttributeOptions($data)
    {
        ...
        foreach ($data['options'] as $index => $option) {
            ...
            $localeFields = wyzo_graphql()->manageLocaleFields($option['translations'], ['label']);
