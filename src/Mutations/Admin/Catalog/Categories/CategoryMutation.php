...
namespace Wyzo\GraphQLAPI\Mutations\Admin\Catalog\Categories;
...
class CategoryMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            wyzo_graphql()->uploadImage($category, $imageUrl, 'category/', 'logo_path');

            wyzo_graphql()->uploadImage($category, $bannerPath, 'category/', 'banner_path');

            return [
                'success'  => true,
                'message'  => trans('wyzo_graphql::app.admin.catalog.categories.create-success'),
                'category' => $category,
            ];
        } catch (\Exception $e) {
    ...
    public function update(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $category) {
            throw new CustomException(trans('wyzo_graphql::app.admin.catalog.categories.not-found'));
        }
        ...
        wyzo_graphql()->validate($args, [
            $locale.'.slug' => ['required', new Slug, function ($value, $fail) use ($args) {
                if (! $this->categoryRepository->isSlugUnique($args['id'], $value)) {
                    $fail(trans('wyzo_graphql::app.admin.catalog.categories.already-taken'));
                }
            }],
        ...
        try {
            ...
            return [
                'success'  => true,
                'message'  => trans('wyzo_graphql::app.admin.catalog.categories.update-success'),
                'category' => $category,
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (strtolower($category->name) == 'root') {
            throw new CustomException(trans('wyzo_graphql::app.admin.catalog.categories.root-delete'));
        }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.catalog.categories.delete-success'),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
        }
