...
namespace Wyzo\GraphQLAPI\Mutations\Admin\Sales\Invoices;
...
class InvoiceMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $order->canInvoice()) {
            throw new CustomException(trans('wyzo_graphql::app.admin.sales.invoices.creation-error'));
        }

        try {
            if (empty($args['invoice_data'])) {
                throw new CustomException(trans('wyzo_graphql::app.admin.sales.invoices.product-error'));
            }
            ...
            return [
                'success'  => true,
                'message'  => trans('wyzo_graphql::app.admin.sales.invoices.create-success'),
                'invoice'  => $invoice,
            ];
        } catch (\Exception $e) {
