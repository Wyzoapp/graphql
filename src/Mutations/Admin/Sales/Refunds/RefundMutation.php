...
namespace Wyzo\GraphQLAPI\Mutations\Admin\Sales\Refunds;
...
use Wyzo\Sales\Repositories\RefundRepository;
...
class RefundMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $order->canRefund()) {
            throw new CustomException(trans('wyzo_graphql::app.admin.sales.refunds.creation-error'));
        }
        ...
        try {
            ...
            if (! isset($refundAmount)) {
                throw new CustomException(trans('wyzo_graphql::app.admin.sales.refunds.refund-amount-error'));
            }

            if ($refundAmount > $maxRefundAmount) {
                throw new CustomException(trans('wyzo_graphql::app.admin.sales.refunds.refund-limit-error', ['amount' => core()->formatBasePrice($maxRefundAmount)]));
            }
            ...
            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.sales.refunds.create-success'),
                'refund'  => $refund,
            ];
        } catch (\Exception $e) {
