...
class PushNotificationMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            wyzo_graphql()->uploadImage($notification, $imageUrl, 'notification/', 'image_url');

            return [
                'success'           => true,
                'message'           => trans('wyzo_graphql::app.admin.settings.notification.create-success'),
    ...
    public function update(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $notification) {
            throw new CustomException(trans('wyzo_graphql::app.admin.settings.notification.not-found'));
        }
        ...
        try {
            ...
            return [
                'success'           => true,
                'message'           => trans('wyzo_graphql::app.admin.settings.notification.update-success'),
                'push_notification' => $notification,
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function sendNotification(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (is_null(core()->getConfigData('general.api.pushnotification.private_key'))) {
            throw new CustomException(trans('wyzo_graphql::app.admin.settings.notification.configuration-error'));
        }
        ...
        if (isset($result->message_id)) {
            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.settings.notification.edit.notification-send-success'),
            ];
        } else {
            $message = $result;
