...
class ChannelMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            wyzo_graphql()->uploadImage($channel, $logo, 'channel/', 'logo');

            wyzo_graphql()->uploadImage($channel, $favicon, 'channel/', 'favicon');

            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.settings.channels.create-success'),
                'channel' => $channel,
            ];
        } catch (\Exception $e) {
    ...
    public function update(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $channel) {
            throw new CustomException(trans('wyzo_graphql::app.admin.settings.channels.not-found'));
        }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.settings.channels.update-success'),
                'channel' => $channel,
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if ($this->channelRepository->count() == 1) {
            throw new CustomException(trans('wyzo_graphql::app.admin.settings.channels.last-delete-error'));
        }

        if ($channel->code == config('app.channel')) {
            throw new CustomException(trans('wyzo_graphql::app.admin.settings.channels.default-delete-error'));
        }
        ...
        try {
            ...
            return [
                'success' => true,
                'message' => trans('wyzo_graphql::app.admin.settings.channels.delete-success'),
            ];
        } catch (\Exception $e) {
            throw new CustomException(trans('wyzo_graphql::app.admin.settings.channels.delete-failed'));
        }
    }
}
