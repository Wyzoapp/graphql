...
use Wyzo\Customer\Repositories\WishlistRepository;
...
class WishlistMutation extends Controller
{
    ...
    public function store(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $product) {
            throw new CustomException(trans('wyzo_graphql::app.shop.customers.account.wishlist.product-removed'));
        }
        ...
        try {
            if ($this->wishlistRepository->findOneWhere($data)) {
                return [
                    'success'  => false,
                    'message'  => trans('wyzo_graphql::app.shop.customers.account.wishlist.already-exist'),
            ...
            return [
                'success'  => true,
                'message'  => trans('wyzo_graphql::app.shop.customers.account.wishlist.success'),
                'wishlist' => $this->wishlistRepository->findWhere([
                    'customer_id' => auth()->user()->id,
                ]),
            ];
    ...
    public function delete(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            if ($wishlist = $this->wishlistRepository->findOneWhere($data)) {
                ...
                return [
                    'success'  => true,
                    'message'  => trans('wyzo_graphql::app.shop.customers.account.wishlist.remove-success'),
                    'wishlist' => $this->wishlistRepository->findWhere([
                        'customer_id' => auth()->user()->id,
                    ]),
                ];
            ...
            return [
                'success'  => false,
                'message'  => trans('wyzo_graphql::app.shop.customers.account.wishlist.not-found'),
                'wishlist' => $this->wishlistRepository->findWhere([
                    'customer_id' => auth()->user()->id,
                ]),
            ];
    ...
    public function move(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            return [
                ...
                'message'  => $result
                    ? trans('wyzo_graphql::app.shop.customers.account.wishlist.moved-to-cart')
