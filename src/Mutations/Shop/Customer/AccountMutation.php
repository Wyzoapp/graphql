        ...
        try {
            ...
            if (! empty($args['current_password'])) {
                if (Hash::check($args['current_password'], $customer->password)) {
                    ...
                    throw new CustomException(trans('wyzo_graphql::app.shop.customers.account.profile.password-unmatch'));
                }
            } else {
            ...
            if ($customer = $this->customerRepository->update($args, $customer->id)) {
                ...
                if (
                    ...
                    wyzo_graphql()->saveImageByURL($customer, $args, 'image_url');
                }

                return [
                    'success'  => true,
                    'message'  => trans('wyzo_graphql::app.shop.customers.account.profile.update-success'),
                    'customer' => $customer,
                ];
            } else {
                throw new CustomException(trans('wyzo_graphql::app.shop.customers.account.profile.update-fail'));
            }
        } catch (\Exception $e) {
        ...
        try {
            if (Hash::check($args['password'], $customer->password)) {
                if ($customer->orders->whereIn('status', [Order::STATUS_PENDING, Order::STATUS_PROCESSING])->first()) {
                    return [
                        'success' => false,
                        'message' => trans('wyzo_graphql::app.shop.customers.account.profile.order-pending'),
                    ];
                } else {
                    ...
                    return [
                        'success' => true,
                        'message' => trans('wyzo_graphql::app.shop.customers.account.profile.delete-success'),
                    ];
                }
            }
            ...
            return [
                'success' => false,
                'message' => trans('wyzo_graphql::app.shop.customers.account.profile.wrong-password'),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
        }
