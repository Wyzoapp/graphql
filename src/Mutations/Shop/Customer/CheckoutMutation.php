...
use Wyzo\Customer\Repositories\CustomerAddressRepository;
use Wyzo\GraphQLAPI\Repositories\NotificationRepository;
...
use Wyzo\Sales\Transformers\OrderResource;
use Wyzo\Shipping\Facades\Shipping;
...
class CheckoutMutation extends Controller
{
    ...
    public function saveCartAddresses(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        wyzo_graphql()->validate($args, $rules);

        if (
            ! auth()->guard('api')->check()
            && ! Cart::getCart()->hasGuestCheckoutItems()
        ) {
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.addresses.guest-address-warning'));
        }
        ...
        if ($cart->haveStockableItems()) {
            if (! $rates = Shipping::collectRates()) {
                throw new CustomException(trans('wyzo_graphql::app.shop.checkout.something-wrong'));
            }
            ...
            return [
                'message'          => trans('wyzo_graphql::app.shop.checkout.addresses.address-save-success'),
                'cart'             => Cart::getCart(),
    ...
    public function shippingMethods(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! $cart) {
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.empty-cart'));
        }
        ...
        if (
            ...
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.invalid-guest-user'));
        }

        if (empty($cart->shipping_address->id)) {
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.no-address-found'));
        }
        ...
        if ($cart->haveStockableItems()) {
            if (! $rates = Shipping::collectRates()) {
                throw new CustomException(trans('wyzo_graphql::app.shop.checkout.shipping.method-not-found'));
            }
            ...
            return [
                'message'          => trans('wyzo_graphql::app.shop.checkout.shipping.method-fetched'),
                'cart'             => Cart::getCart(),
                'shipping_methods' => $shipping_methods,
                'jump_to_section'  => 'shipping',
            ];
            ...
            return [
                'message'         => trans('wyzo_graphql::app.shop.checkout.payment.method-fetched'),
                'cart'            => Cart::getCart(),
                'payment_methods' => Payment::getPaymentMethods(),
                'jump_to_section' => 'payment',
            ];
    ...
    public function saveShipping(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            if (
                ...
                throw new CustomException(trans('wyzo_graphql::app.shop.checkout.shipping.save-failed'));
            }

            Cart::collectTotals();

            return [
                'message'         => trans('wyzo_graphql::app.shop.checkout.shipping.save-success'),
                'cart'            => Cart::getCart(),
                'jump_to_section' => 'payment',
            ];
        } catch (\Exception $e) {
    ...
    public function paymentMethods(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            if (
                ...
                throw new CustomException(trans('wyzo_graphql::app.shop.checkout.payment.method-not-found'));
            }
    ...
    public function savePayment(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            if (
                ...
                throw new CustomException(trans('wyzo_graphql::app.shop.checkout.payment.save-failed'));
            }

            Cart::collectTotals();

            return [
                'message'         => trans('wyzo_graphql::app.shop.checkout.payment.save-success'),
                'cart'            => Cart::getCart(),
        ...
        try {
            if (strlen($args['code'])) {
                ...
                if (! $coupon) {
                    return [
                        'success' => false,
                        'message' => trans('wyzo_graphql::app.shop.checkout.coupon.invalid-code'),
                        'cart'    => Cart::getCart(),
                    ];
                }
                ...
                if ($coupon->cart_rule->status) {
                    if (Cart::getCart()->coupon_code == $args['code']) {
                        return [
                            'success' => true,
                            'message' => trans('wyzo_graphql::app.shop.checkout.coupon.already-applied'),
                            'cart'    => Cart::getCart(),
                        ];
                    }
                    ...
                    if (Cart::getCart()->coupon_code == $args['code']) {
                        return [
                            'success' => true,
                            'message' => trans('wyzo_graphql::app.shop.checkout.coupon.apply-success'),
                            'cart'    => Cart::getCart(),
                        ];
                    }
                }
    ...
    public function removeCoupon(mixed $rootValue, array $args, GraphQLContext $context)
    {
        try {
            if (Cart::getCart()->coupon_code) {
                ...
                return [
                    'success' => true,
                    'message' => trans('wyzo_graphql::app.shop.checkout.coupon.remove-success'),
                    'cart'    => Cart::getCart(),
                ];
            }
            ...
            return [
                'success' => false,
                'message' => trans('wyzo_graphql::app.shop.checkout.couponremove-failed'),
                'cart'    => Cart::getCart(),
            ];
        } catch (\Exception $e) {
            throw new CustomException($e->getMessage());
    ...
    public function saveOrder(mixed $rootValue, array $args, GraphQLContext $context)
    {
        try {
            if (Cart::hasError()) {
                throw new CustomException(trans('wyzo_graphql::app.shop.checkout.error-placing-order'));
            }
    ...
    public function validateOrder()
    {
        ...
        if (
            ...
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.missing-shipping-address'));
        }

        if (! $cart->billing_address) {
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.missing-billing-address'));
        }
        ...
        if (
            ...
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.missing-shipping-method'));
        }

        if (! $cart->payment) {
            throw new CustomException(trans('wyzo_graphql::app.shop.checkout.missing-payment-method'));
        }
    }
     ...
     *
     * @param  \Wyzo\Sales\Contracts\Order  $order
     * @return mixed
     */
