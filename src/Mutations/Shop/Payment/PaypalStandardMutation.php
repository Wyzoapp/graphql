...
namespace Wyzo\GraphQLAPI\Mutations\Shop\Payment;

use Illuminate\Support\Str;
use Nuwave\Lighthouse\Support\Contracts\GraphQLContext;
use Wyzo\Checkout\Facades\Cart;
use Wyzo\GraphQLAPI\Validators\CustomException;
use Wyzo\Paypal\Helpers\Ipn as IpnHelper;
use Wyzo\Paypal\Payment\Standard;
...
class PaypalStandardMutation
{
    ...
    public function redirect(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        if (! isset($company->id)) {
            throw new CustomException(trans('wyzo_graphql::app.admin.response.invalid-comapny-header'));
        } else {
        ...
        try {
            ...
            if (
                ...
                throw new CustomException(trans('wyzo_graphql::app.shop.payment.invalid-request'));
            } else {
                if (
                    ...
                    return [
                        'success'             => true,
                        'message'             => trans('wyzo_graphql::app.shop.payment.paypal-standard.success-form-field', ['module_name' => 'Paypal Standard']),
                        'cart'                => $cart,
                        'paypal_redirect_url' => $this->paypalStandard->getPaypalUrl(),
                        'paypal_form_field'   => $paypalFields,
                    ];
                } else {
                    throw new CustomException(trans('wyzo_graphql::app.shop.payment.paypal-standard.disable-module', ['module_name' => 'Paypal Standard']));
                }
            }
        } catch (\Exception $e) {
        ...
        try {
            ...
            if (
                ...
                if ($order) {
                    return [
                        'success'      => true,
                        'message'      => trans('wyzo_graphql::app.shop.payment.paypal-standard.success-order-place'),
                        'order'        => $order,
                        'redirect_url' => request()->server('PROTOCOL').'://'.$company->domain.'/checkout/success',
                    ];
                } else {
                    throw new CustomException(trans('wyzo_graphql::app.shop.payment.paypal-standard.enable-order-place'));
                }
            } else {
                throw new CustomException(trans('wyzo_graphql::app.shop.payment.paypal-standard.enable-order-place'));
            }
    ...
    public function cancel(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            return [
                'success'      => true,
                'message'      => trans('wyzo_graphql::app.shop.payment.paypal-standard.warning-order-cancel'),
    ...
    public function ipn(mixed $rootValue, array $args, GraphQLContext $context)
    {
        ...
        try {
            ...
            if ($response == false) {
                throw new CustomException(trans('wyzo_graphql::app.shop.payment.paypal-standard.warning-order-cancel'));
            }
