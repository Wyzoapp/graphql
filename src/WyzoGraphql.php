...
namespace Wyzo\GraphQLAPI;

use Illuminate\Support\Facades\Storage;
use Illuminate\Support\Facades\Validator;
use Illuminate\Support\Str;
use Wyzo\GraphQLAPI\Validators\CustomException;
use Wyzo\Product\Repositories\ProductBundleOptionProductRepository;
use Wyzo\Product\Repositories\ProductBundleOptionRepository;
use Wyzo\Product\Repositories\ProductCustomerGroupPriceRepository;
use Wyzo\Product\Repositories\ProductDownloadableLinkRepository;
use Wyzo\Product\Repositories\ProductDownloadableSampleRepository;
use Wyzo\Product\Repositories\ProductGroupedProductRepository;
use Wyzo\Product\Repositories\ProductImageRepository;
use Wyzo\Product\Repositories\ProductVideoRepository;
...
class WyzoGraphql
{
    ...
    public function authorize(string $guard = 'api', ?string $token = null): mixed
    {
        if (! auth()->guard($guard)->check()) {
            throw new CustomException(trans('wyzo_graphql::app.shop.customers.no-login-customer'));
        }
        ...
        if (
            ...
            $message = trans('wyzo_graphql::app.shop.customers.login.suspended');
        }
        ...
        switch ($product->type) {
            ...
            case 'downloadable':
                if (! empty($data['links'])) {
                    ...
                    if (empty($data['links'])) {
                        throw new CustomException(trans('wyzo_graphql::app.shop.checkout.cart.item.error.downloadable-links'));
                    }
                }
                break;
